...
public:
    ...
    Image(const Image& other){
        ...
        //space for copy
        data = reinterpret_cast<stbi_uc *>(stbi__malloc(w * h));    //w*h*c = w*h
...
int main(int argc, const char** argv) {
    ...
    */
    ...
    //normal copy
    Image copy = inp;
    copy.write_png("copy.png");
    //copy with 100 different pixel
    Image copy2 = inp;
    for(int i = 1; i <= 100; i++){
        copy2.set(i, i, 255);
    }
    copy2.write_png("copy2.png");

    //move
    Image move1 = std::move(inp);
    //check move (should be blume2.png: 0x0) 
    std::cout << fn << ": " << inp.getWidth() << "x" << inp.getHeight() << "\n"; 
